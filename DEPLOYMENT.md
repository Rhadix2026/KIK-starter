...
| Staging     | `staging` | :5177    | :8013   | `kik_staging`  | `:staging` |
| Productie   | `main`    | :5176    | :8012   | `kik_starter`  | `:vX.Y.Z`  |
...
3. Valideer op `http://<server>:5177`.
4. PR `staging` → `main`.
...
De productie-frontend draait op poort `5176`. Koppel die later via de reverse proxy
(nginx/Caddy) van de Rhadix-website aan een (sub)domein, net als bij de
validation-app — bijvoorbeeld `kik.rhadix.nl` → `127.0.0.1:5176`. De frontend
